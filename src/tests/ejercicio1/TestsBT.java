...
//			Datos1.toConsole("ficheros/p4/multiconjuntos" + id_fichero + ".txt");

			// V�rtices clave

			Vertex1 start = Vertex1.start();
			...
			System.out.println("Voraz = "+r.getWeight()+"  == "+Solucion1.create(r));
			
			BT<Vertex1, Edge1, Solucion1> bta = BT.of(graph,
					Solucion1::create, null, null, true);

			if (rr.isSolution(r)) {
				bta = BT.of(graph, Solucion1::create, r.getWeight(), r, true);
			}
			Optional<GraphPath<Vertex1, Edge1>> gp = bta.search();
			System.out.println(Solucion1.create(gp.get()));
